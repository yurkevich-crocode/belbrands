        ...
        <div class="dashboard__items">
          ...
          <div class="dashboard__list-items">
            <a href="/brands.php" class="dashboard__list-item"
              ...
              >Бренды </a
            ><a href="/category.php" class="dashboard__list-item"
              ><span class="dashboard__icon-item"
                ><svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
              ...
              >Категории </a
            ><a href="/users.php" class="dashboard__list-item  dashboard__list-item--active"
              ><span class="dashboard__icon-item"
                ><svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
        ...
        <div class="container">
          <div class="users__title-wrapper">
            ...
            <div class="searchbar__wrapper">
              <input name="search" id="search" class="searchbar__input" placeholder="Поиск по пользователям" />
            </div>
          </div>
          <div class="users__wrapper">
            <div class="users__items">
              ...
              $users = mysqli_fetch_all($data_query);


              foreach($users as $user){
              ?>
              <div class="users__item">
                <span class="users__item-id"><?=$user[0]?></span>
                <div class="users__item-img-wrapper"><img src=<?=$user[4]?> class="users__item-img" /></div>
                <div class="users__item-wrapper">
                  <p class="users__name"><?=$user[1]?> <?=$user[2]?></p>
                  <?
                    if($_SESSION['email'] == $user[3]){
                    ?>
                    <span class="users__status">Вы</span> 
                    <?}?>                   
                  <p><?=$user[3]?></p>
                  <div class="users__control-wrapper">
                    <?php
                    if($_SESSION['role'] == "Модератор"){
                      if($user[6] != "Администратор"){
                    ?>
                    <span class="users__control-btn btn-edit"
                    data-name="<?=$user[1]?>"
                    data-last="<?=$user[2]?>"
                  ...
                  <span class="users__status">Администратор</span> 
          ...
          <div class="users__info">
            <div class="users__info-wrapper">
              <span class="users__cross"></span>
              <form class="users__form-inputs" action="php/updateUser.php" method="post" enctype="multipart/form-data">
              <input type="hidden" name="idUser" id="idUser">
              <input type="hidden" name="curEmailUser" id="curEmailUser" value="<?=$_SESSION['email']?>">
                <div class="users__form-input-wrapper">
                  <span class="users__form-input-label">Имя:</span>
                  <input name="userName" id="userName" class="users__form-input" required/>
                </div>
                <div class="users__form-input-wrapper">
                  <span class="users__form-input-label">Фамилия:</span>
                  <input name="userLastName" id="userLastName" class="users__form-input" required/>
                </div>
                <div class="users__form-input-wrapper">
                  <span class="users__form-input-label">Электронная почта:</span>
                  <input type="email" name="userEmail" id="userEmail" class="users__form-input" required/>
                </div>
                <div class="users__form-input-wrapper">
                  <span class="users__form-input-label">Пароль:</span>
                  <input type="current-password" name="userPass" id="userPass" class="users__form-input" required/>
                </div>
                <div class="users__form-input-wrapper">
                  <span class="users__form-input-label">Роль:</span>
                  <select name="roleEdit" id="roleEdit" class="users__form-input" required >
                    <?
                    $roles = mysqli_fetch_all($data_roles);
                    ...
                    <?}
                    if($_SESSION['role'] == "Модератор"){
                      if($role[1] == "Модератор" || $role[1]=="Пользователь"){?>
                    <option value="<?=$role[0]?>" ><?=$role[1]?></option>
                ...
                <button type="submit" class="users__btn">Сохранить изменения</button>
              </form>
            </div>
          </div>
        </div>
